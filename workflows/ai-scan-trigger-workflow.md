...
The end-to-end flow when a user triggers an AI scan. AI scanning is a new scanner-type that slots into the existing scan dispatch pipeline; this doc focuses on what's different from a standard SAST scanner trigger.
...
sequenceDiagram
    ...
    U->>API: POST /api/v1/scans<br/>{scanner_ids: ["ai"], ai_mode: "structured",<br/>ai_provider: "managed-claude"}
    ...
    API->>AI: POST /scans/{scan_id}/ai-trigger<br/>X-Internal-Service-Token
    AI->>DB: Atomic reserve token budget<br/>UPDATE orgs SET tokens_used += est WHERE tokens_used + est <= cap
    alt Reservation failed (over quota)
        AI->>DB: UPDATE scans SET status=failed, failure_type=ai_quota_exceeded
        AI-->>API: 200 {dispatched: false, failure_type: ai_quota_exceeded}
        API-->>U: 200 (scan persisted as failed)
    end
    alt Provider = managed-claude
...
|---|---|---|---|
| `ai_org_disabled` | Org has not opted in | "AI scanning not enabled for your organization" | No |
| `ai_contract_blocked` | Contract has `ai_processing_disabled=true` | "This contract is flagged 'no AI processing'" | No |
| `ai_token_cap_exceeded` | Estimated input > per-scan cap | "Contract too large for your tier's per-scan token cap" | No |
| `ai_quota_exceeded` | Atomic reservation failed | "AI scan quota exceeded — upgrade or wait for reset" | No |
| `ai_key_invalid` | BYO key rejected by provider | "Your BYO API key was rejected by Anthropic / OpenAI / Gemini" | No |
| `ai_provider_error` | Provider returned 5xx or network error | "AI provider returned an error" | Refunded |
| `ai_safety_blocked` | Provider safety filter triggered | "AI provider refused the request (safety filter)" | Charged (input only) |
| `ai_output_invalid` | Output schema validation failed | "AI returned malformed output — no findings recorded" | Charged |
| `ai_system_error` | ai-scanner internal failure | "AI scan service is temporarily unavailable" | Refunded |
| `ai_canceled` | User canceled mid-scan | "Scan canceled" | Charged for what was streamed |

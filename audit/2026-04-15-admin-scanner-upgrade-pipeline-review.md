...
|---|-----|----------|-------------------|-------|
| **1** | **Validate `target_version`** — strict semver regex on `ScannerUpgradeRequest`; validate `scanner_name` exists in the `scanner-versions` ConfigMap before proxying | CRITICAL | `secure-coding.md` A03, `api-endpoint-auth.md` | api-service only |
| 2 | **Image-existence probe** — tool-integration runs `docker manifest inspect` (or equivalent GCP Artifact Registry API call) against target image before ConfigMap patch; 404 → reject with explicit error | MEDIUM | `testing-deployment.md` | tool-integration |
| 3 | **Rollback endpoint** — `POST /api/v1/admin/system/scanners/{name}/rollback`; stores previous version in a new `scanner_version_history` DB table on every upgrade; rollback reads last row and re-runs upgrade with that version | HIGH | `database-management.md` (new table = migration + backup first), `testing-deployment.md` | api-service + tool-integration |
| 4 | **Fill GITHUB_REPOS for missing 8 scanners** — no upstream GitHub releases for Apogee-built scanners (rustdefend, soliditydefend) → add a `developer` field pointing at the internal repo's releases API | MEDIUM | `dependency-management.md` (version policy) | tool-integration |
| 5 | **Unit-of-work across DB + ConfigMap** — wrap tool-integration ConfigMap call + DB upsert in a retry-with-idempotency pattern; if DB write fails, re-apply ConfigMap with the *old* version (compensating action) | MEDIUM | `database-management.md` (consistency), `core-development-rules.md` | api-service |
| 6 | **Notification hook** — on upgrade success/failure, post to `blocksecops-notification` with scanner/previous/new/admin/outcome; admins + platform-ops Slack see it within 60s | LOW | `service-availability.md` (observability) | api-service |

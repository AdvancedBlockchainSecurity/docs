...
|---|------|-----------------|--------|
| 1.1.1 | Supabase JWT login (email/password) | Token issued, session created | [ ] |
| 1.1.2 | OAuth providers: Google, GitHub, Microsoft, Discord | All providers authenticate successfully | [ ] |
| 1.1.3 | Wallet auth: MetaMask, WalletConnect, Phantom (Solana) | Signature verified, session created | [ ] |
| 1.1.4 | HS256 fallback when Supabase unavailable | Local JWT auth works | [ ] |
| 1.1.5 | MFA enforcement (if applicable) | Second factor required | [ ] |

### 1.2 Session Management

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 1.2.1 | Session token expiry | Token expires at configured TTL | [ ] |
| 1.2.2 | Session invalidation on password change | Old sessions terminated | [ ] |
| 1.2.3 | Session invalidation on tier change | Must re-login | [ ] |
| 1.2.4 | Concurrent session limits enforced | Max sessions per user respected | [ ] |
| 1.2.5 | HttpOnly cookie storage (no localStorage tokens) | XSS-safe token storage | [ ] |
| 1.2.6 | Secure and SameSite cookie attributes | Cookies set with Secure + SameSite=Lax/Strict | [ ] |

### 1.3 API Key Authentication

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 1.3.1 | API key auth via `X-API-Key` header (Growth+ tier) | Authenticated, `last_used_at` updated | [ ] |
| 1.3.2 | Expired API key rejected | 401 Unauthorized | [ ] |
| 1.3.3 | Revoked/soft-deleted API key rejected | 401 Unauthorized | [ ] |
| 1.3.4 | API key refresh: old key immediately invalidated | Old key fails, new key works | [ ] |
| 1.3.5 | API key scopes enforced (read-only key can't write) | 403 Forbidden on out-of-scope action | [ ] |

### 1.4 Service Account Authentication

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 1.4.1 | Service account auth via `X-Service-Account-Key` header | Authenticated and scoped | [ ] |
| 1.4.2 | Service account creation (Growth+ admin-only) | Key with `bso_sa_` prefix created | [ ] |
...
| 2.1.4 | Admin portal: only admin users can access | Non-admin returns 403 | [ ] |
...
| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 2.2.1 | Write endpoints use `require_auth_with_scope()` | Scope violations return 403 | [ ] |
| 2.2.2 | API key scope-to-endpoint mapping verified | All endpoints mapped per standards | [ ] |
...
|---|------|------|-------|-----------------|--------|
| 3.1.1 | Developer: scan beyond monthly limit | Developer | 3/month | 402 quota error | [ ] |
| 3.1.2 | Team: scan beyond monthly limit | Team | 15/month | 402 quota error | [ ] |
| 3.1.3 | Growth: scan beyond monthly limit | Growth | 50/month | 402 quota error | [ ] |
| 3.1.4 | Enterprise: unlimited scans | Enterprise | Unlimited | No quota block | [ ] |

### 3.2 Feature Gate Enforcement

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 3.2.1 | Developer/Team: attempt API key creation | Denied (Growth+ only) | [ ] |
| 3.2.2 | Developer/Team: attempt service account creation | Denied (Growth+ only) | [ ] |
| 3.2.3 | Developer: attempt IDE token generation | Denied (Team+ only) | [ ] |
...
| 3.2.7 | Non-Enterprise: JIRA integration | Denied (Enterprise only) | [ ] |
...
| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 3.3.1 | Tier upgrade mid-cycle: new features accessible immediately | No stale cache | [ ] |
| 3.3.2 | Tier downgrade mid-cycle: session invalidated, quotas updated | Immediate enforcement | [ ] |
| 3.3.3 | Tier downgrade: API keys/service accounts revoked | Keys become inactive | [ ] |
| 3.3.4 | DB ENUM constraint: invalid tier value via raw SQL | Rejected by DB | [ ] |

### 3.4 Rate Limiting by Tier

| # | Test | Tier | Rate Limit | Expected Result | Status |
|---|------|------|------------|-----------------|--------|
| 3.4.1 | Developer rate limit | Developer | Per config | 429 after threshold | [ ] |
| 3.4.2 | Team rate limit | Team | Per config | 429 after threshold | [ ] |
| 3.4.3 | Growth rate limit | Growth | 300/min, 10k/hour | 429 after threshold | [ ] |
...
| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 4.1.1 | All write endpoints require authentication | No unauthenticated writes | [ ] |
| 4.1.2 | Public endpoints inventory reviewed | Only health/public endpoints exposed | [ ] |
| 4.1.3 | Admin endpoints require admin role | Non-admin returns 403 | [ ] |
| 4.1.4 | Internal service endpoints require `X-Internal-Service-Key` | External access blocked | [ ] |

### 4.2 Input Validation

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 4.2.1 | Request body size limits enforced | Oversized payloads rejected (413/422) | [ ] |
| 4.2.2 | Query parameter validation (types, ranges) | Invalid params return 422 | [ ] |
| 4.2.3 | Path parameter injection attempts | Sanitized or rejected | [ ] |
| 4.2.4 | File upload: only allowed types (.sol/.vy/.rs/.zip) | Malicious file types rejected | [ ] |
| 4.2.5 | Chat/text input maxLength=4000 enforced | Oversized input rejected | [ ] |
| 4.2.6 | JSON schema validation on all POST/PUT/PATCH bodies | Malformed JSON rejected | [ ] |

### 4.3 Response Security

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 4.3.1 | Error responses: no stack traces or internal details | Generic error messages only | [ ] |
| 4.3.2 | No sensitive data in error responses (passwords, keys, tokens) | Sanitized error output | [ ] |
| 4.3.3 | Pagination: max page size enforced | Cannot request unlimited results | [ ] |
| 4.3.4 | Response headers: no server version disclosure | Server header absent or generic | [ ] |

### 4.4 API Versioning & Documentation

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 4.4.1 | API versioned under `/api/v1/` | All endpoints follow prefix | [ ] |
| 4.4.2 | OpenAPI/Swagger docs available at `/docs` | Spec renders correctly | [ ] |
| 4.4.3 | Deprecated endpoints removed or flagged | No undocumented legacy endpoints | [ ] |
...
|---|---------|----------|---------------|------------------|--------|
| 5.1.1 | Slither | Solidity | | [ ] | [ ] |
| 5.1.2 | Mythril | Solidity | | [ ] | [ ] |
| 5.1.3 | Semgrep | Solidity | | [ ] | [ ] |
| 5.1.4 | SolidityDefend | Solidity | | [ ] | [ ] |
| 5.1.5 | Solhint | Solidity | | [ ] | [ ] |
| 5.1.6 | Aderyn | Solidity | | [ ] | [ ] |
| 5.1.7 | Wake | Solidity | | [ ] | [ ] |
| 5.1.8 | Echidna (fuzzer) | Solidity | | [ ] | [ ] |
| 5.1.9 | Medusa (fuzzer) | Solidity | | [ ] | [ ] |
| 5.1.10 | Halmos (formal) | Solidity | | [ ] | [ ] |
| 5.1.11 | Vyper compiler | Vyper | | [ ] | [ ] |
| 5.1.12 | Slither-Vyper | Vyper | | [ ] | [ ] |
| 5.1.13 | Moccasin | Vyper | | [ ] | [ ] |
| 5.1.14 | Cargo audit | Rust/Solana | | [ ] | [ ] |
| 5.1.15 | Sol-azy | Rust/Solana | | [ ] | [ ] |
| 5.1.16 | RustDefend | Rust/Solana | | [ ] | [ ] |
| 5.1.17 | Trident | Rust/Solana | | [ ] | [ ] |
| 5.1.18 | Cargo-fuzz-solana | Rust/Solana | | [ ] | [ ] |

### 5.2 Scanner Configuration & Security

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 5.2.1 | All scanner images use immutable semantic version tags | No `:latest` tags in production | [ ] |
| 5.2.2 | All images pulled from private Artifact Registry / Harbor | No public registry pulls | [ ] |
| 5.2.3 | 4naly3er (deprecated Dec 2025) excluded from scanner list | Not in UI or API | [ ] |
| 5.2.4 | Scanner image vulnerability scanning (Trivy/Harbor) | No critical CVEs in scanner images | [ ] |
| 5.2.5 | Scanner pod security context: non-root, read-only FS | Security context enforced | [ ] |
| 5.2.6 | Scanner resource limits (CPU, memory) set | No unbounded resource usage | [ ] |

### 5.3 Scanner Selection & Execution

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 5.3.1 | User selects subset of scanners | Only selected scanners execute | [ ] |
| 5.3.2 | Language auto-detection (Solidity, Vyper, Rust) | Correct scanners presented | [ ] |
| 5.3.3 | Fuzzers (Echidna, Medusa, Halmos) require `requires_project: true` | Single-file upload blocked with clear error | [ ] |
| 5.3.4 | Scanner timeout handling | Graceful timeout, auto-retry | [ ] |
| 5.3.5 | Scanner crash mid-execution | Error captured, other scanners unaffected | [ ] |
...
| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 6.1.1 | Upload contract -> select scanners -> initiate scan | Scan job created successfully | [ ] |
| 6.1.2 | Scan status progression: pending -> running -> completed | Status updates in real-time via WebSocket | [ ] |
| 6.1.3 | Scan results: findings normalized to common schema | All scanner output parsed | [ ] |
| 6.1.4 | Multi-scanner comparison view | Comparison renders with dedup grouping | [ ] |
| 6.1.5 | Scan history: previous scans retrievable | Results persist and are queryable | [ ] |

### 6.2 Scan Pod Lifecycle

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 6.2.1 | Scanner pod lifecycle: job created -> runs -> completes -> cleaned up | No orphan pods after scan | [ ] |
| 6.2.2 | Scanner pod scheduling: 10 scans queued simultaneously | Pods scheduled, no starvation | [ ] |
| 6.2.3 | Scanner pod cleanup CronJob | Completed/failed pods cleaned | [ ] |

### 6.3 Report Generation

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 6.3.1 | Basic vulnerability report (all tiers) | Report generated with findings | [ ] |
| 6.3.2 | Compliance report (SOC 2 mapping, Team+) | Compliance sections populated | [ ] |
| 6.3.3 | Audit-ready PDF report (Growth+) | PDF with remediation guidance | [ ] |
...
| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 7.1.1 | Same vuln from Slither + Mythril | Grouped into single dedup group | [ ] |
| 7.1.2 | Fingerprint types: code, location, AST, semantic | All 4 fingerprint columns populated | [ ] |
| 7.1.3 | Automatic dedup grouping at scan ingest time | No manual trigger needed | [ ] |
| 7.1.4 | Multilevel matching: exact + fuzzy + semantic | Correct grouping at each level | [ ] |
...
| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 7.3.1 | Risk scoring: produces score and risk level | Score calculated (CRITICAL/HIGH/MEDIUM/LOW) | [ ] |
| 7.3.2 | Vulnerability classification to BVD codes | Correct category assigned | [ ] |
| 7.3.3 | AI inline explanations (Claude API) | Explanation rendered on detail page | [ ] |
| 7.3.4 | Prompt injection prevention in contract comments | Sanitized before LLM call | [ ] |
| 7.3.5 | BVD pattern seed: 393+ patterns loaded | All patterns in DB | [ ] |
| 7.3.6 | Scanner-to-pattern mappings: 637+ mappings | Count matches expected | [ ] |

### 7.4 Dedup Maintenance

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 7.4.1 | Stale group cleanup (>90 days inactive) | Cleaned by maintenance job | [ ] |
| 7.4.2 | Low-confidence group flagging | Flagged for review | [ ] |
| 7.4.3 | Duplicate group merge | Merged into canonical group | [ ] |
| 7.4.4 | CronJob execution: 6-hour schedule | All tasks complete without deadlock | [ ] |
...
| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 8.2.1 | Valid Stripe webhook signature | Event processed | [ ] |
| 8.2.2 | Invalid/tampered Stripe webhook signature | Rejected with 400 | [ ] |
| 8.2.3 | Webhook tier metadata validation | Tier matches Stripe product | [ ] |
| 8.2.4 | Webhook idempotency: duplicate event processed once | No double-charge or double-provision | [ ] |
...
| 8.4.2 | x402 credit balance tracking | Balance accurate after scan | [ ] |
...
| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 9.1.1 | GitHub OAuth: connect, list repos, disconnect | Full lifecycle works | [ ] |
| 9.1.2 | GitLab OAuth: connect, list repos, disconnect | Full lifecycle works | [ ] |
| 9.1.3 | Bitbucket OAuth: connect, list repos, disconnect | Full lifecycle works | [ ] |
...
| 9.2.3 | Jenkins: OAuth token exchange, trigger build | Pipeline triggered | [ ] |

### 9.3 ChatOps / Notification Channels

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 9.3.1 | Slack webhook: test notification delivery | Message received in channel | [ ] |
| 9.3.2 | Discord webhook: test notification delivery | Message received in channel | [ ] |
| 9.3.3 | Teams webhook: test notification delivery | Message received in channel | [ ] |
| 9.3.4 | Email notifications (scan complete, critical finding) | Email delivered | [ ] |
| 9.3.5 | Webhook message history: view past deliveries | History displayed with status | [ ] |
...
| 9.4.2 | JIRA: non-Enterprise tier attempts connection | Denied with tier gate message | [ ] |

### 9.5 IDE Integrations

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 9.5.1 | IDE token generation (Team+): create, display once | Token works for IDE auth | [ ] |
| 9.5.2 | IDE token: retrieve token value after creation | Only prefix shown (security) | [ ] |
...
| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 10.1.1 | SQL injection: parameterized queries on all endpoints | No injection possible | [ ] |
| 10.1.2 | NoSQL injection (if applicable) | Queries parameterized | [ ] |
| 10.1.3 | Command injection via contract filenames | Filenames sanitized | [ ] |
| 10.1.4 | LDAP injection (if applicable) | Input sanitized | [ ] |

### 10.2 XSS & CSRF

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 10.2.1 | Stored XSS: malicious contract name/description | Output encoded | [ ] |
| 10.2.2 | Reflected XSS: malicious URL parameters | Output encoded | [ ] |
| 10.2.3 | No `localStorage` auth tokens (removed in v0.45.8) | Tokens in HttpOnly cookies only | [ ] |
| 10.2.4 | CSRF: state-changing requests require valid token | Forged requests rejected | [ ] |
...
| 10.3.2 | File upload: malicious file types rejected | Only .sol/.vy/.rs/.zip allowed | [ ] |
| 10.3.3 | Path traversal in file upload/download | Traversal blocked | [ ] |

### 10.4 Security Headers

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 10.4.1 | Content-Security-Policy header present | CSP configured correctly | [ ] |
| 10.4.2 | X-Frame-Options: DENY | Clickjacking prevented | [ ] |
| 10.4.3 | Strict-Transport-Security (HSTS) | HSTS enabled with max-age | [ ] |
| 10.4.4 | X-Content-Type-Options: nosniff | MIME sniffing prevented | [ ] |
| 10.4.5 | Referrer-Policy configured | No sensitive data in referrer | [ ] |

### 10.5 CORS Configuration

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 10.5.1 | CORS: requests from authorized origins only | Correct origins allowed | [ ] |
| 10.5.2 | CORS: requests from unauthorized origins blocked | No Access-Control-Allow-Origin | [ ] |
| 10.5.3 | No CORS wildcard (`*`) with credentials | Wildcard + credentials rejected | [ ] |
...
| 11.1.3 | No breaking migration without data migration script | Data preserved | [ ] |

### 11.2 Constraint Enforcement

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 11.2.1 | ENUM constraints: invalid tier/status values rejected | DB-level enforcement | [ ] |
| 11.2.2 | Audit log triggers: INSERT works, UPDATE/DELETE blocked | Append-only verified | [ ] |
| 11.2.3 | Foreign key cascades: delete org -> cascades correctly | Related records cleaned | [ ] |
| 11.2.4 | Soft delete consistency: `is_active=false` excluded from queries | No ghost data | [ ] |
| 11.2.5 | Unique constraints: duplicate entries rejected | DB-level enforcement | [ ] |

### 11.3 Database Performance

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 11.3.1 | Index performance: key queries use indexes | EXPLAIN shows index scan | [ ] |
| 11.3.2 | Connection pool under sustained load | No connection exhaustion | [ ] |
| 11.3.3 | Query performance: critical queries < 100ms | SLA met | [ ] |
| 11.3.4 | PostgreSQL SSL enabled | All service connections use TLS | [ ] |
...
| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 12.1.1 | All 8 services: `runAsNonRoot: true`, `runAsUser: 1000` | Pod spec verified | [ ] |
| 12.1.2 | All containers: `readOnlyRootFilesystem`, `drop ALL` capabilities | Security context enforced | [ ] |
| 12.1.3 | Seccomp profile: `RuntimeDefault` on all pods | Profile applied | [ ] |
| 12.1.4 | No privileged containers | `privileged: false` verified | [ ] |
...
| 12.2.3 | Ingress controller: only Traefik routes external traffic | No direct pod access | [ ] |

### 12.3 Deployment Configuration

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 12.3.1 | `revisionHistoryLimit: 3` on all deployments | No stale ReplicaSets | [ ] |
| 12.3.2 | Resource limits (CPU, memory) set on all containers | No unbounded usage | [ ] |
| 12.3.3 | Liveness/readiness probes configured | Health checks active | [ ] |
...
| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 12.4.1 | All images from private registry (Harbor/Artifact Registry) | No public registry pulls | [ ] |
| 12.4.2 | Immutable image tags (no `:latest` in production) | Semantic version tags only | [ ] |
...
| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 13.1.1 | HTTPS enforced on all external endpoints | HTTP redirects to HTTPS | [ ] |
| 13.1.2 | TLS 1.2+ minimum version | No TLS 1.0/1.1 | [ ] |
| 13.1.3 | Valid TLS certificates (production) | No cert errors | [ ] |
| 13.1.4 | PostgreSQL SSL enabled for all service connections | `hostssl` enforced | [ ] |
...
| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 14.1.1 | All secrets stored in HashiCorp Vault / GCP Secret Manager | No plaintext secrets in Git | [ ] |
| 14.1.2 | ExternalSecret resources sync from Vault | Kubernetes secrets populated | [ ] |
| 14.1.3 | Secret rotation: rotate a secret -> services pick up new value | No downtime during rotation | [ ] |
| 14.1.4 | No secrets in Docker images | `docker history` shows no secrets | [ ] |
| 14.1.5 | No secrets in container environment variables (non-secret ConfigMaps) | Only non-sensitive values | [ ] |
| 14.1.6 | Secrets audit: grep codebase for hardcoded secrets | No hardcoded credentials | [ ] |
...
| 14.2.3 | Password hashing: bcrypt with cost factor >= 12 | Correct algorithm verified | [ ] |
| 14.2.4 | API key hashing: SHA-256 (not stored plaintext) | Only hash stored | [ ] |
| 14.2.5 | No prohibited algorithms (MD5, SHA-1 for security, DES, RC4) | Compliant algorithms only | [ ] |
...
|---|------|--------|----------|--------|
| 15.1.1 | Health endpoints: p95 response time | < 100ms | | [ ] |
| 15.1.2 | Scan list/detail endpoints: p95 response time | < 500ms | | [ ] |
| 15.1.3 | Vulnerability list: p95 response time | < 500ms | | [ ] |
| 15.1.4 | Dashboard initial page load | < 3s | | [ ] |
| 15.1.5 | Authentication endpoints: p95 response time | < 300ms | | [ ] |
...
| # | Test | Target | Measured | Status |
|---|------|--------|----------|--------|
| 15.3.1 | API service CPU: sustained load | < 80% utilization | | [ ] |
| 15.3.2 | API service memory: no leaks over 1-hour test | Stable memory usage | | [ ] |
...
| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 16.1.1 | `/health/live` on all 8 services | 200 OK with status | [ ] |
| 16.1.2 | `/health/ready` includes dependency checks | DB, Redis, external deps | [ ] |
...
| 16.2.2 | Structured JSON logging with severity levels | Log queries filterable | [ ] |
| 16.2.3 | No sensitive data in logs (passwords, tokens, PII) | Log sanitization verified | [ ] |
| 16.2.4 | Audit log: authentication events, tier changes, admin actions | Events recorded | [ ] |
...
| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 17.1.1 | Feature branch -> PR -> review -> merge workflow enforced | No direct commits to main | [ ] |
...
| 17.1.5 | Docker image built with immutable tag in CI | Semantic version enforced | [ ] |
...
| 17.2.3 | Rollback procedure tested | Previous version deployable | [ ] |
| 17.2.4 | No credentials in CI/CD configuration files | Secrets via CI/CD secrets manager | [ ] |
...
| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 18.1.1 | Automated daily database backups | Backups created on schedule | [ ] |
| 18.1.2 | Backup integrity: restore from latest backup | Full data recovery | [ ] |
| 18.1.3 | Backup retention: N-day retention policy enforced | Old backups cleaned | [ ] |
...
| 18.2.2 | Pod self-healing: kill API service pod -> auto-restart | No data loss, auto-recovery | [ ] |
| 18.2.3 | PersistentVolume: PostgreSQL data survives pod restart | Data intact | [ ] |
...
| 19.1.4 | Audit trail: all security events logged and immutable | Append-only audit log | [ ] |

### 19.2 Data Privacy (GDPR)

| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 19.2.1 | ML data consent: GDPR opt-in/out tracked | Consent state respected | [ ] |
...
| 20.1.2 | Full scan lifecycle: upload -> select scanners -> scan -> results -> dedup -> report | Complete flow, no errors | [ ] |
...
| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 20.4.1 | Kill API service pod -> auto-restart -> no data loss | Self-healing verified | [ ] |
...
| # | Test | Expected Result | Status |
|---|------|-----------------|--------|
| 21.1.1 | All pods Running, no CrashLoopBackOff | All pods healthy | [ ] |
| 21.1.2 | All `/health` endpoints respond | 200 OK across services | [ ] |
| 21.1.3 | Database connectivity: API -> CloudSQL | Queries succeed | [ ] |
| 21.1.4 | Redis connectivity: cache operations | SET/GET succeed | [ ] |
| 21.1.5 | External secrets synced | All ExternalSecret resources show Synced | [ ] |
...
| 21.2.2 | Auth flow: login via Supabase, receive JWT | Authenticated session | [ ] |
| 21.2.3 | Scan flow: upload + scan + results displayed | End-to-end works | [ ] |
| 21.2.4 | Stripe: pricing page loads, checkout redirects | Payment flow accessible | [ ] |
| 21.2.5 | Admin portal: accessible to admin users only | RBAC enforced | [ ] |
...
| Section | Total Tests | Passed | Failed | Partial | Not Tested |
|---------|-------------|--------|--------|---------|------------|
| 1. Authentication & Identity | | | | | |
| 2. Authorization & Access Control | | | | | |
| 3. Tier System & Quota Enforcement | | | | | |

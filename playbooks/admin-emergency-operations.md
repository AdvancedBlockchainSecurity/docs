...
curl -sk -X POST -H "Authorization: Bearer ${TOKEN}" \
  -H "X-Admin-Session: ${ADMIN_SESSION}" \
  -H "Content-Type: application/json" \
  "https://app.0xapogee.com/api/v1/admin/emergency/revoke-admin/${USER_ID}" \
...
curl -sk -H "Authorization: Bearer ${TOKEN}" \
  -H "X-Admin-Session: ${ADMIN_SESSION}" \
  "https://app.0xapogee.com/api/v1/admin/audit/admin-actions?page=1&page_size=20" | jq '.entries[] | {action, admin_email: .admin_user.email, target_type, created_at}'
```

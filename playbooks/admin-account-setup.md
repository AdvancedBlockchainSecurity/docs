...
- Access to the server via `https://app.0xapogee.com` (server) or `http://127.0.0.1:3000` (minikube)
...
curl -sk -X POST -H "Authorization: Bearer ${TOKEN}" \
  ...
  -d "{\"code\": \"${TOTP_CODE}\"}" \
  "https://app.0xapogee.com/api/v1/admin/auth/mfa/verify" | jq '.'
```
...
curl -sk -H "Authorization: Bearer ${TOKEN}" \
  -H "X-Admin-Session: ${ADMIN_SESSION}" \
  "https://app.0xapogee.com/api/v1/admin/users?page=1&page_size=5" | jq '.'
```

...
curl -sk -X POST "https://app.blocksecops.local/api/v1/api-keys" \
...
```bash
# Create a key
RESULT=$(curl -sk -X POST "https://app.blocksecops.local/api/v1/api-keys" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
curl -sk "https://app.blocksecops.local/api/v1/contracts" \
  -H "X-API-Key: $API_KEY"
```
...
curl -sk "https://app.blocksecops.local/api/v1/vulnerabilities" \
  -H "X-API-Key: $API_KEY"
```
...
# Check initial state
curl -sk "https://app.blocksecops.local/api/v1/api-keys/$KEY_ID" \
...
# Revoke the key
curl -sk -X DELETE "https://app.blocksecops.local/api/v1/api-keys/$KEY_ID" \
  -H "Authorization: Bearer $TOKEN"
...
# Refresh the key
RESULT=$(curl -sk -X POST "https://app.blocksecops.local/api/v1/api-keys/$KEY_ID/regenerate" \
  -H "Authorization: Bearer $TOKEN")
...
```bash
# List all keys
curl -sk "https://app.blocksecops.local/api/v1/api-keys" \

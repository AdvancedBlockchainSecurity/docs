...
| Access URL | https://app.blocksecops.local |
...
curl -sk -X POST -H "Authorization: Bearer ${TOKEN}" \
  -H "X-Admin-Session: ${ADMIN_SESSION}" \
  -H "Content-Type: application/json" \
  "https://app.blocksecops.local/api/v1/admin/auth/logout" \
  -w "\nHTTP_CODE:%{http_code}"
```
...
curl -sk -H "Authorization: Bearer ${TOKEN}" \
  -H "X-Admin-Session: ${ADMIN_SESSION}" \
  "https://app.blocksecops.local/api/v1/admin/users?page=1&page_size=10" | jq '.'
```
...
curl -sk -H "Authorization: Bearer ${TOKEN}" \
  -H "X-Admin-Session: ${ADMIN_SESSION}" \
  "https://app.blocksecops.local/api/v1/admin/users/${USER_ID}" | jq '.'
```
...
curl -sk -X PATCH -H "Authorization: Bearer ${TOKEN}" \
  -H "X-Admin-Session: ${ADMIN_SESSION}" \
  -H "Content-Type: application/json" \
  "https://app.blocksecops.local/api/v1/admin/users/${USER_ID}" \
  -d '{"tier": "team"}' | jq '.'
```
...
curl -sk -X POST -H "Authorization: Bearer ${TOKEN}" \
  -H "X-Admin-Session: ${ADMIN_SESSION}" \
  -H "Content-Type: application/json" \
  "https://app.blocksecops.local/api/v1/admin/users/${USER_ID}/disable" \
...
curl -sk -H "Authorization: Bearer ${TOKEN}" \
  -H "X-Admin-Session: ${ADMIN_SESSION}" \
  "https://app.blocksecops.local/api/v1/admin/audit/admin-actions?page=1&page_size=5" | jq '.'
```
...
curl -sk -X POST -H "Authorization: Bearer ${TOKEN}" \
  -H "X-Admin-Session: ${ADMIN_SESSION}" \
  -H "Content-Type: application/json" \
  "https://app.blocksecops.local/api/v1/admin/emergency/revoke-sessions/${USER_ID}" \
...
**URL**: `https://app.blocksecops.local/admin/mfa-verify`
...
**URL**: `https://app.blocksecops.local/admin`
...
**URL**: `https://app.blocksecops.local/admin/users`
...
**URL**: `https://app.blocksecops.local/admin/emergency`

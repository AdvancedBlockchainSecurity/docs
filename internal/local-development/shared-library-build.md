...
This document details the build process for the `blocksecops-shared` library, a high-performance multi-language foundation that provides consistent types, utilities, and business logic across Rust, Python, and TypeScript services.
...
```bash
make build-rust
# Result: rust/target/release/libblocksecops_shared.{dylib,rlib}
```
...
python3 -c "
import blocksecops_shared as ss
...
# Generates: target/wheels/blocksecops_shared-*.whl
...
```bash
# Python services
pip install blocksecops-shared

# TypeScript/Node.js services
npm install blocksecops-shared

# Rust services
cargo add blocksecops-shared
```
...
RUN pip install blocksecops-shared
...
python3 -c "import blocksecops_shared; print('Success!')"
```

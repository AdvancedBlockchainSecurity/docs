...
> **NOTE:** With Traefik migration, port 3000 is the primary port for dashboard and API access. See [Dashboard Development Standards](/docs/standards/dashboard-development.md) for the correct setup.

### Symptoms
```bash
# Traefik port-forward fails (most common)
$ kubectl port-forward -n traefik-local svc/traefik 3000:80
error: unable to listen on port 3000: Listeners failed to create with the following errors:
[unable to create listener: Error listen tcp4 127.0.0.1:3000: bind: address already in use]

# Or other port-forward failures
...
```bash
# Kill specific port (e.g., port 3000 for Traefik)
...
**Restart Traefik port-forward** (primary method):
```bash
...
# Verify working
sleep 2
curl -s http://127.0.0.1:3000/api/v1/health/live | jq '.status'
```

**Use different local port** (only for debugging specific services):
```bash
...
> **⚠️ WARNING:** Do NOT run `npm run dev` locally for the dashboard. The dashboard runs inside Minikube and is accessed via Traefik on port 3000.
...
See [Local Development Setup Standards](/docs/standards/local-development-setup.md) for complete minikube configuration requirements.

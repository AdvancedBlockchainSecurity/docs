...
│  blocksecops-shared (Rust + Python PyO3 + TypeScript)   │
└─────────────────────────────────────────────────────────────────┘
                                  │
    ┌─────────────────────────────┼─────────────────────────────┐
    │                             │                             │
...
```json
{
  "dependencies": {
    ...
    "@blocksecops/shared": "file:../blocksecops-shared/typescript"
  }
}
```
...
```json
{
  "compilerOptions": {
    ...
    "paths": {
      ...
      "@/hooks/*": ["./src/hooks/*"],
      "@/utils/*": ["./src/utils/*"],
      "@shared/*": ["../blocksecops-shared/typescript/src/*"]
    }
  },
  "include": ["src", "vite.config.ts"],
  "references": [
    { "path": "./tsconfig.node.json" },
    { "path": "../blocksecops-shared/typescript" }
  ]
}
```
...
npm install file:../blocksecops-shared/typescript
```
...
```toml
[package]
name = "blocksecops-contract-parser"
version = "0.1.0"
edition = "2021"
rust-version = "1.75.0"
description = "High-performance Solidity contract parsing service"
license = "MIT"
repository = "https://github.com/your-org/blocksecops-contract-parser"
...
# Shared library integration
blocksecops-shared = { path = "../blocksecops-shared/rust" }
...
```bash
# Verify installation
node -e "const { Vulnerability } = require('@blocksecops/shared'); console.log('Success')"
```
...
sed -i "s/version = .*/version = \"$VERSION\"/" blocksecops-shared/python/pyproject.toml

# Update TypeScript version
sed -i "s/\"version\": .*/\"version\": \"$VERSION\",/" blocksecops-shared/typescript/package.json

# Update Rust version
sed -i "s/version = .*/version = \"$VERSION\"/" blocksecops-shared/rust/Cargo.toml
```

...
#### pytest Configuration (Supabase Authentication)

**Updated November 16, 2025** - Migrated to Supabase authentication with JWT tokens.

```python
# conftest.py - Shared test configuration with Supabase auth
import os
import time
import pytest
import asyncio
...
def authenticated_client(client):
    """
    Create an authenticated test client with Supabase JWT token.

    This fixture:
    1. Creates a unique test user directly in the database
    2. Generates a valid Supabase-format JWT token using HS256
    3. Sets the Authorization header with Bearer token

    Note: Uses HS256 for local testing. Production uses ES256 with Supabase JWKS.
    """
    # Generate unique test user credentials
    user_id = str(uuid.uuid4())
    email = f"test_{int(time.time())}@example.com"

    # Hash password (required by schema, not used for JWT auth)
    ph = PasswordHasher()
    hashed_password = ph.hash("test-password-not-used")

    # Get database connection
    db_url = os.getenv("DATABASE_URL", "postgresql://postgres:postgres@localhost:5432/solidity_security")
    engine = create_engine(db_url)
    SessionLocal = sessionmaker(bind=engine)
    db_session = SessionLocal()

    try:
        # Create test user with quota
        db_session.execute(
            text("""
                INSERT INTO users (id, email, hashed_password, is_active, tier, supabase_user_id)
                VALUES (:id, :email, :hashed_password, true, 'free', :supabase_user_id)
            """),
            {"id": user_id, "email": email, "hashed_password": hashed_password, "supabase_user_id": user_id}
        )

        # Create user quota (auto-created by trigger in production)
        db_session.execute(
            text("""
                INSERT INTO user_quotas (
                    user_id, tier, monthly_scan_limit, monthly_scans_used,
                    max_files_per_scan, scan_priority, webhooks_enabled,
                    api_access_enabled, quota_reset_at
                )
                VALUES (
                    :user_id, 'free', 10, 0, 25, 25, false, false,
    ...
    # Generate HS256 JWT token (Supabase format)
    secret_key = os.getenv("JWT_SECRET_KEY", "local-dev-jwt-secret-key-change-in-production")
    payload = {
        "sub": user_id,
        "email": email,
        "aud": "authenticated",
...
**Key Changes from Legacy Authentication:**
- ❌ **Old**: Used `/auth/register` endpoint (removed in Supabase migration)
- ✅ **New**: Direct database user creation with proper schema
- ❌ **Old**: Cookie-based session authentication
- ✅ **New**: JWT Bearer token authentication
- ✅ **New**: HS256 fallback for local testing without Supabase instance

**JWT Token Format:**
- **Production**: ES256 with Supabase JWKS (ECDSA P-256 asymmetric signing)
- **Local Testing**: HS256 with JWT_SECRET_KEY (symmetric signing)
- **Fallback Logic**: Automatic detection based on "kid" header field presence
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      '@shared': path.resolve(__dirname, '../blocksecops-shared/typescript/src')
    }
  }
})
```
...
vi.mock('@blocksecops/shared', async () => {
  const actual = await vi.importActual('@blocksecops/shared')
  return {
    ...actual,
...
import { Vulnerability, Severity } from '@blocksecops/shared'
...
test.describe('Dashboard', () => {
  ...
  test('shared library integration works', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      const { generateContractId } = await import('@blocksecops/shared')
...
def test_json_compatibility():
    ...
    ts_script = f"""
    const {{ Vulnerability, Severity }} = require('@blocksecops/shared');
    ...
    rust_test = f"""
    use blocksecops_shared::{{Vulnerability, Severity}};
    let vuln = Vulnerability {{
        title: "Cross-Language Test".to_string(),
        severity: Severity::High,
        confidence: 0.95,
      ...
      - name: Python Code Quality
        run: |
          ...
          for service in api-service tool-integration intelligence-engine orchestration data-service notification; do
            echo "Checking blocksecops-$service..."
...
import { Vulnerability, Severity, Location } from '@blocksecops/shared'

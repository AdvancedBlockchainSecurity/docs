...
# Check Job status
kubectl get jobs -n blocksecops
kubectl get pods -n blocksecops -l job-name=scan-slither-test123

# Get logs
kubectl logs -n blocksecops job/scan-slither-test123

# Cleanup
kubectl delete job scan-slither-test123 -n blocksecops
kubectl delete configmap scan-test123-source -n blocksecops
```
...
kubectl get configmaps -n blocksecops -l app=scanner

# Get ConfigMap details
kubectl describe configmap scan-{id}-source -n blocksecops

# View ConfigMap content
kubectl get configmap scan-{id}-source -n blocksecops -o yaml
```

### Check Scanner Jobs
```bash
# List scanner Jobs
kubectl get jobs -n blocksecops -l app=scanner

# Check Job status
kubectl describe job scan-slither-{id} -n blocksecops

# Get scanner logs
kubectl logs -n blocksecops -l scan-id={id}
```
...
**Check**:
```bash
kubectl get configmaps -n blocksecops
...
```bash
kubectl auth can-i create configmaps --as=system:serviceaccount:blocksecops:tool-integration -n blocksecops
```
...
kubectl delete configmaps -n blocksecops -l app=scanner
```
...
**Check**:
```bash
kubectl describe pod {scanner-pod} -n blocksecops | grep -A 10 Mounts
kubectl exec -n blocksecops {scanner-pod} -- ls -la /contracts/
```
...
**Check**:
```bash
kubectl describe pod {scanner-pod} -n blocksecops
```
...
### ConfigMap Access
- ConfigMaps created in `blocksecops` namespace

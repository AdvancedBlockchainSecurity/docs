    ...
    "spec": {
      "template": {
        "spec": {
          "containers": [{
            "name": "dependency-monitor",
            "image": "blocksecops-dependency-monitor:latest"
          }]
        }
      }
    }
...
- **Build Image Locally**:
  ...
  cd /Users/pwner/Git/ABS/blocksecops-monitoring/dependency-monitor
  docker build -t blocksecops-dependency-monitor:latest .

  # For minikube
  minikube image load blocksecops-dependency-monitor:latest
  ```
...
```bash
curl -X POST http://localhost:8080/scan/api-service
# {"error": "Repository not found", "details": "Path /app/repos/blocksecops-api-service not accessible"}
```
...
kubectl exec deployment/dependency-monitor -n monitoring-local -- ls -la /app/repos/blocksecops-api-service/
...
- **Update Service Configuration**:
  ...
  kubectl patch configmap dependency-monitor-config -n monitoring-local -p '{
    "data": {
      "service_paths.json": "{\"api-service\":\"/app/repos/blocksecops-api-service\",\"ui-core\":\"/app/repos/blocksecops-ui-core\"}"
    }
  }'
  ```
